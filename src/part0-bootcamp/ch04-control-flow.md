...
Розглянемо просчтий цикл:



**Змінна `i`** знищується і **створюється заново** кожну ітерацію.
...
```rust
for i in 1..=3 {
    // ітерація 1: створюється змінна i1 = 1
    // ітерація 2: створюється змінна i2 = 2
    // ітерація 3: створюється змінна i3 = 3
}
```

Кожна ітерація — це **новий блок** з **новою змінною** `i`, яка отримує наступне значення з ітератора.

### Доказ — спробуйте зробити mut

```rust
for i in 1..=3 {
    i += 10;  // ✗ Помилка: cannot assign to `i`, as it is not mutable
    println!("{}", i);
}
```

Але навіть якщо додати `mut`:
...
Зміна `i` не впливає на наступні ітерації — бо це **різні змінні**.
